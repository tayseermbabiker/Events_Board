...
const LISTING_URL = 'https://www.terrapinn.com/events/middle-east';

const UAE_CITIES = ['Dubai', 'Abu Dhabi', 'Sharjah', 'Ajman', 'Ras Al Khaimah', 'Fujairah', 'Umm Al Quwain'];

class TerrapinnScraper extends BaseScraper {
  constructor() {
    super('Terrapinn');
  }
  ...
  async scrape() {
    ...
    for (const stub of stubs) {
      try {
        ...
        // Also grab the page description and hero image as fallbacks
        const pageMeta = await this.page.evaluate(() => {
          const desc = document.querySelector('meta[name="description"]');
          const img = document.querySelector('.hero img, .banner img, .event-hero img, og\\:image, meta[property="og:image"]');
          ...
          return {
            ...
            image: img?.src || img?.content || document.querySelector('meta[property="og:image"]')?.content || null,
          };
        });
        ...
        const locationText = [venueName, venueAddress, stub.venueText].join(' ').toLowerCase();
        ...
        // Skip non-UAE events
        if (!city) {
          logger.info(this.name, `Skipping non-UAE event: ${title}`);
          continue;
        }
        ...
        events.push({
          ...
          image_url: image,
          source: 'Terrapinn',
          source_event_id: `terrapinn-${stub.slug}`,
        });
...
module.exports = TerrapinnScraper;
